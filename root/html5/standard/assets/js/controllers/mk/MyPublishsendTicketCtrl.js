...
app.controller('MyPublishsendTicketCtrl', ["$window", "$scope", "$rootScope", "$location", "ngTableParams", "SweetAlert", function ($window, $scope, $rootScope, $location, ngTableParams, SweetAlert) {

    var data = [];

    $rootScope.currTitle = "已发行卡券";
    ...
    $scope.getticket = function () {
        var options = {
            ...
            url: myforwardurl(serviceRoot2 + 'self/firstSelf?actiontype=getticket&User='+user_id+'&shop_id=&grantStatus=1', serviceRoot2),
            async: false,
            type: 'get',
            dataType: 'json',
            data: null,
    ...
    $scope.newact = function () {
        $location.url("/app/wx/me/mypublish_sendticket_new?shop_id=" + shop_id);
    }


    $scope.editit = function (theid) {

        $location.url("/app/wx/me/mypublish_sendticket_new?theid=" + theid + "&opertype=edit");
    }

    $scope.back_ticket = function (theid,ticket_id) {

        var options = {
            //url:myforwardurl(serviceRoot+'mk_main_info?actiontype=del_ticket&theid='+theid,serviceRoot),
            url: myforwardurl(serviceRoot2 + 'self/firstSelf?actiontype=back_ticket&ids='+theid, serviceRoot2),
            async: false,
            type: 'get',
            dataType: 'json',
    ...
    $scope.sendsms=function () {
        var options = {
            ...
            success: function (rv) {
                ...
                if (isok) {
                    alert("发行成功");
                    $location.path("app/wx/me/mypublish_sendticket");
