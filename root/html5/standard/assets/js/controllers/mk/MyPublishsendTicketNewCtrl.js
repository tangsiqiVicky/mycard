...
app.controller('MyPublishsendTicketNewCtrl', ["$window", "$scope", "$location", "$rootScope", '$localStorage', '$state', '$stateParams', 'ngTableParams', function ($window, $scope, $location, $rootScope, $localStorage, $state, $stateParams, ngTableParams) {
...
//editor end
    ...
    $scope.getticket = function () {
        var options = {
            ...
            url: myforwardurl(serviceRoot2 + 'self/firstSelf?actiontype=getticket&ownerStatus=0&User='+user_id+'&shop_id='+shops_id+'&ticket_type='+$scope.ticket_type+'&grantStatus=1', serviceRoot2),
            async: false,
            type: 'get',
            dataType: 'json',
            data: null,
...
//     actiontype.equals("fafang_ticket")) {  //发放卡券
//     String ticket_ids=s_func.GBK(request.getParameter("ticket_ids"));
// String owner=s_func.GBK(request.getParameter("owner"));
//
...
//func_obj=$scope.open;
    ...
    $scope.sure = function () {
        $.closePopupLayer();
        $("#select").val();
        console.info('$scoe'.$scope.selected );
